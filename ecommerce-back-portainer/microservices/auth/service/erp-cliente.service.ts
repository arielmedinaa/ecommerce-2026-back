...
export class ErpClienteService {
  ...
  constructor(
    private readonly econt: EcontDatabaseService,
  ...
  async getClienteErpByDocumento(documento: string): Promise<ErpClienteDto> {
    ...
    try {
      ...
      if (!c) {
        ...
        try {
          ...
          if (p) {
            ...
            if (firstName || lastName) {
              const local = await this.getLaboralLocal(ruc);
              if (local) {
                return {
                  encontrado: true,
        ...
        const local = await this.getLaboralLocal(ruc);
        if (local) {
          return { encontrado: true, documento: String(ruc), ...local };
        }
      ...
      let laboral: ErpClienteDto['laboral'] = {
      ...
      if (!this.laboralTieneContenido(laboral)) {
        const local = await this.getLaboralLocal(ruc);
        if (local?.laboral) laboral = local.laboral;
      }
  ...
  private async getLaboralLocal(
    ruc: number,
  ): Promise<Partial<ErpClienteDto> | null> {
    try {
      const u = await this.userService.findByNumeroDocumento(String(ruc));
      if (!u?.datosLaborales) return null;
      const d = u.datosLaborales as Record<string, any>;
      const laboral: ErpClienteDto['laboral'] = {
        empresa: this.str(d.empresa),
        cargo: this.str(d.cargo),
        rubro: this.str(d.rubro),
        fechaIngreso: this.str(d.fechaIngreso),
        salario: d.salario != null ? Number(d.salario) : undefined,
        ciudad: this.str(d.ciudad),
        barrio: this.str(d.barrio),
        callePrincipal: this.str(d.callePrincipal),
        calleSecundaria: this.str(d.calleSecundaria),
        numerocasa: this.str(d.numerocasa),
        contactoNombre: this.str(d.contactoNombre),
        contactoTelefono: this.str(d.contactoTelefono),
      };
      if (!this.laboralTieneContenido(laboral)) return null;

      const [firstName, ...rest] = (u.nombre || '').trim().split(/\s+/).filter(Boolean);
      return {
        firstName: firstName || '',
        lastName: rest.join(' '),
        email: u.email || '',
        phone: u.numeroCelular || '',
        laboral,
      };
    } catch (e) {
      this.logger.warn(
        `Fallback local datosLaborales falló (ruc ${ruc}): ${(e as any)?.message ?? e}`,
      );
      return null;
    }
  }

  // Campos que solo importan como "hay contenido" si vienen realmente
  // completados. En el ERP, cuando el laboral no fue cargado, salario/ciudad/
  // numerocasa suelen venir en "0" en vez de vacío, así que esos placeholders
  // numéricos no cuentan como contenido real por sí solos.
  private laboralTieneContenido(
    laboral: ErpClienteDto['laboral'] | undefined,
  ): boolean {
    if (!laboral) return false;
    const camposClave: (keyof NonNullable<ErpClienteDto['laboral']>)[] = [
      'empresa',
      'cargo',
      'rubro',
      'fechaIngreso',
