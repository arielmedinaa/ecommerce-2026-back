...
var CartValidationService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.CartValidationService = void 0;
const common_1 = require("@nestjs/common");
const cart_error_service_1 = require("./errors/cart-error.service");
let CartValidationService = CartValidationService_1 = class CartValidationService {
    constructor(cartErrorService) {
        this.cartErrorService = cartErrorService;
        this.logger = new common_1.Logger(CartValidationService_1.name);
    }
    async validateProduct(producto, codigo) {
        if (!producto) {
            const error = new Error('Producto es null o undefined');
            await this.cartErrorService.logMicroserviceError(error, codigo?.toString(), 'validateProduct', {
    ...
    async validateCartPayload(clienteToken, cuenta, codigo, producto) {
        if (!clienteToken || clienteToken.trim() === '') {
            const error = new Error('Token de cliente inválido');
            await this.cartErrorService.logMicroserviceError(error, codigo?.toString(), 'validateCartPayload', {
    ...
    async validateFinishCart(clienteToken, cuenta, codigo, process) {
        if (!codigo || codigo <= 0) {
            const error = new Error('Código de carrito inválido');
            await this.cartErrorService.logMicroserviceError(error, codigo?.toString(), 'validateFinishCart', {
    ...
    async validateInsertCentralApp(solicitud, clienteToken, codigo, clienteInfo) {
        if (!solicitud || [solicitud].length < 0) {
            const error = new Error('Solicitud inválida - No se proporcionó el objeto de la solicitud');
            await this.cartErrorService.logMicroserviceError(error, codigo?.toString(), 'validateInsertCentralApp', {
...
exports.CartValidationService = CartValidationService = CartValidationService_1 = __decorate([
