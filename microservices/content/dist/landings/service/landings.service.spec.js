...
var LandingValidationService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.LandingValidationService = void 0;
const common_1 = require("@nestjs/common");
const landings_error_service_1 = require("./errors/landings-error.service");
let LandingValidationService = LandingValidationService_1 = class LandingValidationService {
    constructor(landingErrorService) {
        this.landingErrorService = landingErrorService;
        this.logger = new common_1.Logger(LandingValidationService_1.name);
    }
    async validateCreateLanding(createLandingDto, userId) {
        if (!createLandingDto) {
            const error = new Error('Datos de landing son null o undefined');
            await this.landingErrorService.logMicroserviceError(error, userId, 'validateCreateLanding', {
    ...
    async validateUpdateLanding(id, updateLandingDto, userId) {
        if (!id || id.trim() === '') {
            const error = new Error('ID de landing no proporcionado');
            await this.landingErrorService.logMicroserviceError(error, userId, 'validateUpdateLanding', {
                motivo: 'id_null',
                id,
                updateLandingDto,
                userId,
    ...
    async validateFormato(formatoDto, userId) {
        if (!formatoDto) {
            const error = new Error('Datos de formato son null o undefined');
            await this.landingErrorService.logMicroserviceError(error, userId, 'validateFormato', {
    ...
    async validateUpdateFormato(id, updateFormatoDto, userId) {
        if (!id || id.trim() === '') {
            const error = new Error('ID de formato no proporcionado');
            await this.landingErrorService.logMicroserviceError(error, userId, 'validateUpdateFormato', {
    ...
    async validatePaginationParams(page, limit) {
        const defaultPage = 1;
    ...
    async validateTemplateKey(templateKey) {
...
exports.LandingValidationService = LandingValidationService;
exports.LandingValidationService = LandingValidationService = LandingValidationService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [landings_error_service_1.LandingErrorService])
], LandingValidationService);
//# sourceMappingURL=landings.service.spec.js.map