...
export class SellerImageValidatorUtil {
  ...
  private async convertAndUpload(
    ...
    try {
      ...
      return { success: true, url: put.url, message: 'Imagen validada correctamente.' };
    } catch (err: any) {
  ...
  // Usado tanto para imagen_1..5 provistas como URL en el Excel/corrección,
  // como para el flujo de "pegar URL" del proveedor al solucionar un rechazo.
  ...
  // Usado tanto para imágenes incrustadas en la celda del Excel (bytes ya
  // extraídos vía ExcelJS worksheet.getImages()) como para archivos subidos
  // directamente por el proveedor al corregir un rechazo.
