...
- Precio (precioventa) y stock_actual son valores plausibles (no cero,
  no absurdos).
...
307 Precio por debajo del mínimo permitido — precioventa parece
    anormalmente bajo o inválido.
...
export class ProductsSellerAiApprovalService {
  ...
  private async evaluar(seller: ProductsSeller): Promise<VeredictoIA> {
    const payload = {
      ...
      precioventa: Number(seller.precioventa),
