DROP PROCEDURE IF EXISTS proc_obtener_articulos_ecommerce_web_v2test;

DELIMITER $$

CREATE DEFINER=`root`@`%` PROCEDURE `proc_obtener_articulos_ecommerce_web_v2test`(
    IN p_limit INT,
    IN p_offset INT,
...
BEGIN
    ...
    DECLARE v_sin_filtros TINYINT DEFAULT 0;
    ...
    -- Caso patologico: sin busqueda ni ningun filtro selectivo (marca/categoria/
    -- proveedor/precio). Ahi el EXISTS correlacionado contra tbl_stock_actual
    -- evalua ~44K filas de articulo sin poder apoyarse en ningun indice previo
    -- (~0.7-2.9s medido). Con cualquier filtro presente el original ya es rapido
    -- (0.01-0.36s medido) porque esos filtros usan indices que acotan las filas
    -- candidatas antes de llegar al chequeo de stock; forzar el JOIN materializado
    -- ahi de nuevo suma un costo fijo (~0.3s de escanear el indice de stock) que
    -- resulta ser una regresion neta.
    SET v_sin_filtros = IF(
        v_busqueda IS NULL
        AND (p_codigo_marca IS NULL OR TRIM(p_codigo_marca) = '')
        AND (p_codigo_categoria IS NULL OR TRIM(p_codigo_categoria) = '')
        AND (p_codigo_proveedor IS NULL OR TRIM(p_codigo_proveedor) = '')
        AND p_precio_min IS NULL AND p_precio_max IS NULL,
        1, 0
    );
    ...
    DROP TEMPORARY TABLE IF EXISTS tmp_stock_ok;

    IF v_sin_filtros = 1 THEN
        CREATE TEMPORARY TABLE tmp_stock_ok ENGINE=InnoDB AS
        SELECT DISTINCT sa.codigo_articulo
        ...
        ALTER TABLE tmp_stock_ok ADD PRIMARY KEY (codigo_articulo);

        CREATE TEMPORARY TABLE tmp_ids ENGINE=InnoDB AS
        SELECT a.codigo_articulo
        FROM ssss_emp1.articulo a
        JOIN tmp_stock_ok tso ON tso.codigo_articulo = a.codigo_articulo
        LEFT JOIN ssss_emp1.cs_score_proveedor sp ON sp.proveedor = COALESCE(
            (SELECT c.proveedor FROM ssss_emp1.compradet c
             WHERE c.codigo_articulo = a.codigo_articulo
             ORDER BY c.secuencia DESC LIMIT 1),
            a.proveedor)
        LEFT JOIN ssss_emp1.cs_pautas_meta_ticket_rangos tr
            ON tr.activo = 1
           AND a.precioventa >= tr.monto_desde
           AND (tr.monto_hasta IS NULL OR a.precioventa <= tr.monto_hasta)
        WHERE a.baja = 0
          AND (a.websc = 1 OR a.web = 1)
          AND a.precioventa >= 9000
        ORDER BY
            CASE tr.nombre_ticket
    ...
    LEFT JOIN ssss_emp1.cs_score_proveedor sp ON sp.proveedor = COALESCE(tp.proveedor, a.proveedor)
    LEFT JOIN ssss_emp1.cs_pautas_meta_ticket_rangos tr
        ON tr.activo = 1
       AND a.precioventa >= tr.monto_desde
       AND (tr.monto_hasta IS NULL OR a.precioventa <= tr.monto_hasta)
