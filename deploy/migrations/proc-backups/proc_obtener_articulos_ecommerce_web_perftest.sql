DROP PROCEDURE IF EXISTS proc_obtener_articulos_ecommerce_web_perftest;

DELIMITER $$

CREATE DEFINER=`root`@`%` PROCEDURE `proc_obtener_articulos_ecommerce_web_perftest`(
    IN p_limit INT,
    IN p_offset INT,
...
BEGIN
    ...
    -- Set chico y ya materializado de articulos con stock disponible en depositos
    -- habilitados. Evita repetir el EXISTS+JOIN a deposito como subquery
    -- correlacionada sobre cada fila de articulo (~2.9s antes vs ~0.3s asi).
