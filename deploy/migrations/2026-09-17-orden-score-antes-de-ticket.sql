-- 2026-09-17 — Orden del catálogo: score de proveedor ANTES que el tramo de ticket.
--
-- Cambios respecto de 2026-09-14-fix-proc-productos-duplicados.sql:
--   1. Los tres ORDER BY (tmp_ids y los dos finales) pasan de
--        relevancia -> ticket -> tier de proveedor -> score
--      a
--        relevancia -> tier de proveedor -> score -> ticket
--      La regla de negocio es "mejor score primero, y dentro de cada score el
--      ticket más bajo primero"; el orden anterior mezclaba proveedores porque
--      agrupaba por tramo de precio antes de mirar el score.
--   2. El SELECT público expone orden_proveedor_tier, score_proveedor y
--      orden_ticket. Antes el orden viajaba sólo como orden de filas, así que
--      cualquier re-sort en el front (relevancia, precio, JOTA) lo destruía sin
...
BEGIN
    ...
    ORDER BY _match_relevance ASC, _prioridad_tier ASC, _prioridad_score DESC, _tier_ticket ASC, a.nombre ASC, a.codigo_articulo ASC;
    ...
    SELECT
        ...
        _prioridad_tier AS orden_proveedor_tier,
        _prioridad_score AS score_proveedor,
        _tier_ticket AS orden_ticket
    FROM tmp_resultado
    ORDER BY _match_relevance ASC, _prioridad_tier ASC, _prioridad_score DESC, _tier_ticket ASC, nombre_articulo ASC, codigo_articulo ASC;
